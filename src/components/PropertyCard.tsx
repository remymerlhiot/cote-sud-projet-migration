...
const extractNegotiatorName = (fullName?: string): string => {
  ...
  const cleanName = fullName.replace(/&apos;/g, "'").replace(/&#039;/g, "'").replace(/&#39;/g, "'");

  // Supprimer l'agence et autres textes non pertinents
  const agencyVariants = [/L['']Agence immobilière de prestige Côté Sud/i, /L['']Agence Côté Sud/i, /Agence Côté Sud/i, /Côté Sud/i];
      ...
      <Card className="overflow-hidden border-none shadow-md h-full bg-white hover:shadow-lg transition-all duration-300">
        <CardContent className="p-0 relative">
          ...
          <div className="p-4 text-center">
            ...
            {(property.hasBalcony || property.hasTerrasse || property.hasPool || (property.garageCount || "") !== "" || property.bathrooms) && <div className="mt-3 flex flex-wrap justify-center gap-2">
                {property.hasBalcony && <span className="px-2 py-1 bg-gray-100 text-[10px] rounded">
                    Balcon
                  </span>}
                {property.hasTerrasse && <span className="px-2 py-1 bg-gray-100 text-[10px] rounded">
                    Terrasse
                  </span>}
                {property.hasPool && <span className="px-2 py-1 bg-gray-100 text-[10px] rounded">
                    Piscine
                  </span>}
                {property.garageCount && <span className="px-2 py-1 bg-gray-100 text-[10px] rounded">
                ...
                {property.isFurnished && <span className="px-2 py-1 bg-gray-100 text-[10px] rounded">
                    Meublé
                  </span>}
              </div>}
