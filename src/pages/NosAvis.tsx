...
const StarRating = ({
  ...
  return <div className="flex">
      {[...Array(5)].map((_, i) => <Star key={i} className={`h-4 w-4 ${i < rating ? 'text-sable fill-sable' : 'text-gray-300'}`} />)}
    </div>;
};
      ...
      <div className="container mx-auto py-12 px-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full max-w-4xl mx-auto">
          <TabsList className="bg-sable-30 w-full flex justify-center mb-8">
            {agentsWithReviews.map(agent => <TabsTrigger key={agent.id} value={agent.id} className="data-[state=active]:bg-cuivre data-[state=active]:text-white">
                {agent.name}
              </TabsTrigger>)}
          </TabsList>
          
          {agentsWithReviews.map(agent => <TabsContent key={agent.id} value={agent.id} className="mt-4">
              <div className="text-center mb-8">
                <Avatar className="h-24 w-24 mx-auto mb-4 border-2 border-sable">
                  {agent.image ? <AvatarImage src={agent.image} alt={agent.name} /> : <AvatarFallback className="bg-sable-50 text-cuivre text-xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                {agent.reviews.map(review => <Card key={review.id} className="border border-sable-30">
                    <CardContent className="p-6">
                      <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-2">
                          <Avatar className="h-10 w-10">
                            {review.authorImage ? <AvatarImage src={review.authorImage} alt={review.author} /> : <AvatarFallback className="bg-sable-50 text-cuivre">
                                {review.author[0]}
                              </AvatarFallback>}
                          </Avatar>
                          <div>
        ...
        <div className="text-center mt-16">
          ...
          <Button className="bg-cuivre hover:bg-anthracite text-white px-8 py-2" onClick={() => window.open('https://g.page/r/YOURREVIEWLINK/review', '_blank')}>
